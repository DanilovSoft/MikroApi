...
public sealed class MikroTikResponseListener : IMikroTikResponseListener
{
    ...
    {
        Cancel(waitACK: true, cancellationToken);
    }
    ...
    public void Cancel(bool waitACK, CancellationToken cancellationToken = default)
    {
        ThrowIfCanceled();

        // В любом случае нужен собственный тег что-бы результат не попал в основную очередь когда wait = false
        var selfTag = _connection.CreateUniqueTag();
        ...
        if (waitACK)
        {
            ...
            var readerIsStopped = _connection.AddTagToRead(selfTag);

            // Важно захватить блокировку перед отправкой в сокет.
            lock (cancelCommand)
            {
                // Отправить запрос без ожидания ответа.
                _connection.Send(cancelCommand, cancellationToken);

                if (readerIsStopped)
                {
                    // Запуск потока для чтения.
                    _connection.StartBackgroundRead();
                }
    ...
    {
        return CancelAsync(waitACK: true, cancellationToken);
    }
    ...
    public async Task CancelAsync(bool waitACK, CancellationToken cancellationToken = default)
    {
        ThrowIfCanceled();
        SetCanceled(); // Этот Listener больше нельзя отменять.

        if (waitACK)
        {
            // Подписываемся на завершение отмены.
            var cancelCommand = _connection.CreateAsyncCancelCommand(_tag);
            
            var readerIsStopped = _connection.AddTagToRead(cancelCommand.SelfTag); // Добавляем задание для потока читающего из сокета.
           
            // Отправляем тегированный запрос на отмену без ожидания результата.
            await _connection.SendAsync(cancelCommand, cancellationToken).ConfigureAwait(false);
            
            StartRead(readerIsStopped); // Запуск потока для чтения.
        ...
        {
            // Отправляем тегированный запрос на отмену без ожидания результата.
            await _connection.CancelListenerNoWaitAsync(_tag, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    private void StartRead(bool readerIsStopped)
    {
        if (readerIsStopped)
        {
            // Запуск потока для чтения.
